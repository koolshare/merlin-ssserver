...
EOF
	if [ "$ssserver_udp" == "1" ];then
		if [ "ssserver_ota" == 1 ];then
			ss-server -c /koolshare/ssserver/ss.json -u -A -f /tmp/ssserver.pid
		else
			ss-server -c /koolshare/ssserver/ss.json -u -f /tmp/ssserver.pid
		fi
	else
		if [ "ssserver_ota" == 1 ];then
			ss-server -c /koolshare/ssserver/ss.json -A -f /tmp/ssserver.pid
		else
			ss-server -c /koolshare/ssserver/ss.json -f /tmp/ssserver.pid
		fi
	fi
